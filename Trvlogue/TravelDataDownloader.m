...
#import "TravelDataDownloader.h"

#import "NSString+Soundex.h"

#import "DDCurrencyUnitConverter.h"

@implementation TravelDataDownloader
...
- (id)initWithDelegate:(id<TravelDataDelegate>)_delegate andFlightID:(NSString *)_FlightID {
    ...
    if (self) {
        ...
        self.flight = [Database flightFromID:self.FlightID];
    }
...
- (void)fireDataDownload {
    ...
    if (![Database travelDataPacketWithID:self.FlightID]) {
        
        [Database addTravelDataPacketWithID:self.FlightID andTravelDataObject:self];
    }
...
- (void)downloadPeople {
    ...
    [PeopleFeed findPeopleFromFlight:self.FlightID withCompletionHandler:^(NSMutableArray *objects, NSError *error, NSString *callCode) {
        ...
        if (!error) {
            ...
            if (![self.downloadedData[@"people"] containsObject:objects[0]]) {
                ...
                [Database refreshTravelDataPacketWithID:self.FlightID andTravelDataObject:self];
