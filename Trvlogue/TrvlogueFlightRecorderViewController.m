...
@interface TrvlogueFlightRecorderViewController ()

@end

@implementation TrvlogueFlightRecorderViewController
...
- (void)initializeAutocompletionObject {
    ...
    autocompletionSuggestionsRetriever = [[AutocompleteSuggestionsRetriever alloc] init];
}
...
- (void)submitFlight {
    ...
    [LoadingSignifier signifyLoading:@"Recording your flight" duration:-1];
}

- (void)createTrvlogueFlight {
    
    TrvlogueFlight *trvlogueFlight = [[TrvlogueFlight alloc] initWithParameters:flightParameters];

    [self insertTrvlogueFlight:trvlogueFlight];
}

- (void)insertTrvlogueFlight:(TrvlogueFlight *)trvlogueFlight {
...
- (void)updateAccount:(TrvlogueAccount *)updatedAccount {
    
    [[NSNotificationCenter defaultCenter] postNotificationName:@"RecordedFlight" object:nil];
    
    [LoadingSignifier signifyLoading:@"Uploading flight" duration:-1];
    ...
    [Database updateMyAccount:updatedAccount withCompletionHandler:^(BOOL success, NSError *error, NSString *callCode) {
        ...
        if (success && !error) {
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                ...
                [NotificationSignifier signifyNotification:@"Flight has been recorded" forDuration:3];
