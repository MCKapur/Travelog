		...
		for (NSString * row in rows) {
			...
			if (hasStartedCulling == NO) {
				...
				if ([row hasPrefix:@"Currency"]) {
                    ...
                    _DDCurrencyExchangeRates[currencyName] = [NSDecimalNumber numberWithDouble:conversionValue];
					rowIndex++;
				}
			}
		}
...
+ (NSDecimalNumber *) multiplierForUnit:(DDUnit)unit {
	...
	if (unit < DDCurrencyUnitSDR) {
        ...
        multiplier = _DDCurrencyExchangeRates[name];
        if (multiplier == nil) {
            NSLog(@"unknown currency: %@ (%i)", name, unit);
