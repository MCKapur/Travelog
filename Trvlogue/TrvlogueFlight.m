...
- (void)instantiateTravelData {
    ...
    if ([reach isReachable] && ([reach isReachableViaWiFi] || [reach isReachableViaWWAN])) {
        ...
        if ([Database travelDataPacketWithID:self.ID]) {
            
            TravelDataDownloader *travelData = [[TravelDataDownloader alloc] initWithDelegate:self andFlightID:self.ID];
            [travelData setDownloadedData:[Database travelDataPacketWithID:self.ID]];
...
- (NSString *)description {
    ...
    return [NSString stringWithFormat:@"\r%@\r\rMiles:%g\r\rName:%@ to %@\r\rDate:%@\r", self.ID, self.miles, self.originCity, self.destinationCity, [DateString convertDateToString:self.date withFormat:DAY_MONTH_YEAR]];
}

- (NSString *)generateRandomID {
    
    return [Database generateRandomKeyWithLength:15];
}
