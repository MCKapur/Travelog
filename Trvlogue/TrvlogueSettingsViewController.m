...
#import "TrvlogueSettingsViewController.h"

#import "TrvlogueLoginViewController.h"

#import "Database.h"

@interface TrvlogueSettingsViewController ()

@end

@implementation TrvlogueSettingsViewController
...
- (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex {
    ...
    if (buttonIndex == 1) {
        ...
        [LoadingSignifier signifyLoading:@"Sending deactivation request" duration:-1];
        
        NSMutableDictionary *emailOptions = [NSMutableDictionary dictionaryWithObjectsAndKeys:[NSString stringWithFormat:@"Request to delete his/her account. email:%@",[[Database currentAccount] email]], @"data", @"Trvlogue Account Deletion Request", @"subject", VERIFIED_EMAIL_TEST, @"toAddress", nil];
...
- (void)downloadedAccountsFromEmails:(NSMutableArray *)accounts {
    ...
    if (accounts.count) {
        ...
        [ErrorHandler handleError:[NSError errorWithDomain:@"No friends found" code:200 userInfo:@{NSLocalizedDescriptionKey:@"No friends found"}]];
    }
}
...
- (IBAction)logout {
    ...
    TrvlogueLoginViewController *lvc = [[TrvlogueLoginViewController alloc] init];
...
- (void)sendEmailOnDatabase: (NSMutableDictionary *)emailOptions {
    ...
    [Database sendEmail:emailOptions withCompletionHandler:^(BOOL success, NSError *error, NSString *callCode) {
        
        if (success && !error) {
            
            [NotificationSignifier signifyNotification:@"We'll miss you, stay cool :)" forDuration:10];
...
- (void)updateAccount {
    ...
    [LoadingSignifier signifyLoading:@"Saving your account" duration:-1];
    
    TrvlogueAccount *updatedTrvlogueAccount = [Database currentAccount];
    ...
    [Database uploadProfilePicture:profilePicture.image withObjectId:[[PFUser currentUser] objectId]];
...
- (void)updateMyAccount: (TrvlogueAccount *)account {
    
    [Database updateMyAccount:account withCompletionHandler:^(BOOL success, NSError *error, NSString *callCode) {
        
        if (!error && success) {
            
            [NotificationSignifier signifyNotification:@"Changes have been made" forDuration:3];
        }
        else {
...
- (BOOL)hasChangedData {
    ...
    if ([[emailTextField.text stringByReplacingOccurrencesOfString:@" " withString:@""] isEqualToString:[Database currentAccount].email] && [[nameTextField.text stringByReplacingOccurrencesOfString:@" " withString:@""] isEqualToString:[Database currentAccount].person.name] && [profilePicture.image isEqual:[[Database currentAccount].person getProfilePic]] && [passwordTextField.text isEqualToString:[[Database currentAccount] password]]) {
...
- (int)missingFields {
    ...
    if (array.count) {
        ...
        [ErrorHandler handleError:[NSError errorWithDomain:@"Please fill in all fields" code:200 userInfo:@{NSLocalizedDescriptionKey:@"Please fill in all fields"}]];
    }
...
- (int)checkEmailIsValid {
    ...
    if (![self validateEmailWithString:[emailTextField.text stringByReplacingOccurrencesOfString:@" " withString:@""]]) {
        ...
        if (![self missingFields]) {
            ...
            [ErrorHandler handleError:[NSError errorWithDomain:@"Email specified is invalid" code:200 userInfo:@{NSLocalizedDescriptionKey:@"Email specified is invalid"}]];
        }
    }
    else {
...
- (void)loadInAccountData {
    ...
    [nameTextField setText:[[Database currentAccount].person name]];
    [emailTextField setText:[[Database currentAccount] email]];
    [profilePicture setImage:[[Database currentAccount].person getProfilePic]];
    [passwordTextField setText:[[Database currentAccount] password]];
}
