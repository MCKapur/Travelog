...
- (void)initializer {
    ...
    _colorIndicatorView = [[UIView alloc] initWithFrame:CGRectMake(self.bounds.origin.x, self.bounds.origin.y+ 5, self.bounds.size.width, self.bounds.size.height - 17)];
...
- (void)slideImageWithPercentage:(CGFloat)percentage imageName:(NSString *)imageName isDragging:(BOOL)isDragging {
    ...
    slidingImageRect = CGRectMake(position.x - slidingImageSize.width / 2.0,
            position.y - (_colorIndicatorView.center.y + 8) / 2.0,
