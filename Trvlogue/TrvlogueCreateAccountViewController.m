...
@interface TrvlogueCreateAccountViewController ()
{
...
@implementation TrvlogueCreateAccountViewController
...
- (void)createAccount {
    ...
    [LoadingSignifier signifyLoading:@"Creating your account" duration:-1];
                        ...
                        [geocoder geocodeAddressString:[array[i] componentsSeparatedByString:@" to"][1] completionHandler:^(NSArray *placemarks, NSError *error) {
                            ...
                            if (!error) {
                                ...
                                if (CLLocationCoordinate2DIsValid(destinationCoordinate)) {
                                    ...
                                    [flightsArray addObject:[[TrvlogueFlight alloc] initWithParameters:[NSDictionary dictionaryWithObjects:@[originCity, destinationCity, originCountry, destinationCountry, [DateString convertStringToDate:[[NSUserDefaults standardUserDefaults] objectForKey:@"date"][i] withFormat:DAY_MONTH_YEAR], [[NSUserDefaults standardUserDefaults] objectForKey:@"miles"][i], @"", @(originCoordinate.latitude), @(originCoordinate.longitude), @(destinationCoordinate.latitude), @(destinationCoordinate.longitude)] forKeys:@[@"originCity", @"destinationCity", @"originCountry", @"destinationCountry", @"date", @"miles", @"question", @"originLatitude", @"originLongitude", @"destinationLatitude", @"destinationLongitude"]]]];
                                }
                            }
                        }];
                    }
    ...
    [self geocode:[originCityTextField text] withCompletionHandler:^(CLPlacemark *placemark, NSError *error) {
        ...
        if ([location length] && !error) {
            ...
            self.accountDict = [[NSMutableDictionary alloc] initWithObjectsAndKeys:nameTextField.text, @"name", [emailTextField.text stringByReplacingOccurrencesOfString:@" " withString:@""], @"email", /*needs to be fixed*/passwordTextField.text, @"password", followingArray, @"connections", milesNumber, @"miles", flightsArray, @"flights", [[NSMutableDictionary alloc] init], @"knownDestinationPreferences", [placemark locality], @"originCity", [[NSMutableArray alloc] init], @"notifications", jobTextField.text, @"position", @(isUsingLinkedIn), @"isUsingLinkedIn", accessToken, @"linkedInAccessKey", linkedInId, @"linkedInId", nil];
            
            account = [[TrvlogueAccount alloc] initWithProfile:self.accountDict];
            
            [self registerAccount];
        }
        else {
            
            [ErrorHandler handleError:[NSError errorWithDomain:@"Please input a valid city" code:200 userInfo:@{NSLocalizedDescriptionKey:@"Please input a valid city"}]];
        }
    }];
}
...
- (void)registerAccount {
    ...
    [LoadingSignifier signifyLoading:@"Uploading your account" duration:-1];

    dispatch_queue_t downloadQueue = dispatch_queue_create("Upload account", NULL);
    dispatch_async(downloadQueue, ^{
        
        [Database uploadAccount:account withCompletionHandler:^(BOOL success, NSError *error, NSString *callCode) {
...
- (void)loginCorrectCredentials {
    ...
    TrvlogueViewController *trvlogueViewController = [[TrvlogueViewController alloc] init];
...
- (void)viewDidLoad
{
    if (isUsingLinkedIn) {
        ...
        emailTextField.text = presetData[@"emailAddress"];
        nameTextField.text = presetData[@"formattedName"];
    ...
    [Database isCreatingAnAccount:YES];
