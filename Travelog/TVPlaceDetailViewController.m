...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (!self.segmentedControl.selectedSegmentIndex) {
        ...
        if ([info[indexPath.row][@"ID"] isEqualToString:@"Website"]) {
            ...
            NSString *cleanedNumber = [[place.phoneNumber componentsSeparatedByCharactersInSet:[[NSCharacterSet characterSetWithCharactersInString:@"0123456789-+()"] invertedSet]] componentsJoinedByString:@""];
            NSURL *URL = [[NSURL alloc] initWithString:[NSString stringWithFormat:@"tel://%@", cleanedNumber]];
