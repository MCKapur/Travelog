...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section
{
    ...
    if ([uiGridViewDelegate numberOfCellsOfGridView:self]) {
        
        residue = ([uiGridViewDelegate numberOfCellsOfGridView:self] % [uiGridViewDelegate numberOfColumnsOfGridView:self]);
        
        if (residue > 0) residue = 1;
        
        retVal = ([uiGridViewDelegate numberOfCellsOfGridView:self] / [uiGridViewDelegate numberOfColumnsOfGridView:self]) + residue;
    }
	...
	for (int i=0;i<numCols;i++) {
		...
		if ((i + indexPath.row * numCols) >= count) {
			...
			if ([row.contentView.subviews count] > i) {
				((UIGridViewCell *)(row.contentView.subviews)[i]).hidden = YES;
			}
		...
		if ([row.contentView.subviews count] > i) {
			tempCell = (row.contentView.subviews)[i];
		} else {
			tempCell = nil;
		}
