...
-(void)addToWindow
{
    ...
    else [[window subviews][0] addSubview:self.view];
...
-(void)removeAlertFromView
{
    ...
    {
        MBAlertView *alert = displayQueue[0];
        currentAlert = alert;
...
-(void)didSelectButton:(MBAlertViewButton*)button
{
    if(button.tag >= _items.count)
        return;
    MBAlertViewItem *item = _items[button.tag];
    if(!item)
        return;
...
-(void)didTapOutsideOfButtons:(UITapGestureRecognizer *)recognizer
{
    if (recognizer.state == UIGestureRecognizerStateEnded) {
        if(_buttons.count == 1)
        {
            MBAlertViewButton *alertButton = _buttons[0];
